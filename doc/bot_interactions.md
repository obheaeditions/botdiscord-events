...
3. **Mise à Jour des Embeds :** Le message sur lequel l'utilisateur a cliqué contient plusieurs embeds (carte Titre, carte Détails, galerie d'images — voir [bot_publishing.md](bot_publishing.md)). Ils sont tous régénérés via `buildEmbeds()` puis appliqués instantanément en invoquant `interaction.update({ embeds })` ; les statistiques actualisées apparaissent dans la carte Détails.
